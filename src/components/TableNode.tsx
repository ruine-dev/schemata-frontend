  ...
  return (
    <div
      className={clsx(
        ...
        {
          'ring-2 ring-offset-8 ring-sky-500': selected,
        },
      )}
      onKeyDown={(e) => {
        if (e.key === 'Delete') {
        ...
        {table.columns.map((column, index) => (
          <li key={column.id} className="group relative">
            ...
            <Handle
              ...
              position={Position.Right}
              type="target"
              className={clsx('peer absolute -right-[0.3125rem] h-2.5 w-2.5 bg-sky-500 opacity-0', {
                'z-20 group-hover:opacity-100': isTarget,
              })}
            />
            <Handle
            ...
            />
            <TableColumn column={column} tableId={id} className="peer-hover:bg-slate-100" />
          </li>
        ))}
      </ul>
      <button
        ...
        className="nodrag flex w-full items-center justify-center rounded-b-xl py-3 px-3 font-sans text-xs font-medium text-sky-500 outline-none ring-sky-500 hover:bg-sky-50 hover:text-sky-600 focus:ring-2 active:bg-sky-50 active:text-sky-600"
      >
